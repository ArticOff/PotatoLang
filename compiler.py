...
-v --Version    : Display the version.
            ...
            try:
                ...
                with open(file=__File__, mode="r", encoding="utf-8") as __FILE__:
                    ...
                    for LINE in __LINES__:
                        ...
                        for function in list(__FUNCTIONS__):
                            ...
                            if _M == function:
                                ...
                                if not (len(Args) != 0 or len(funcArgs) != 0):
                                    for i in Args:
                                        if i == "":
                                            Args.remove("")
                                    for i in funcArgs:
                                ...
                                if len(Args) != len(funcArgs):
                                    ma = int(str(len(Args) - len(funcArgs)).removeprefix("-")) - 2
                                    syntaxError(f"Missing the argument \"{funcArgs[ma]}\"")
                                ...
                                for o in list(__FUNCTIONS__):
                                    if str(o).startswith("system"):
                                        args = "".join(Args)
                                        os.system(args)
                                    if str(o).startswith("python"):
                                        args = "".join(Args)
                                        exec(args)
                                    else:
                                        if str(funcCode[0]).startswith(o):
                                            ...
                                            for i in argFuncCode:
                                                if str(funcCode[0]).startswith("system"):
                                                    args = "".join(Args)
                                                    os.system(args)
                                                if str(funcCode[0]).startswith("python"):
                                                    args = "".join(Args)
                                                    exec(args)
                        ...
                        if str(LINE).startswith("with"):
                            if str(LINE).split(" ")[1] in __MODULES__:
                                if str(LINE).split(" ")[1] == "system":
                                    ...
                                    funct = {}
                                elif str(LINE).split(" ")[1] == "string":
                                    funct = {}
                                    funct["args"] = ["text", "letter", "variable"]
                                    funct["code"] = ["variable = exec(str(text).split(letter)))", "return variable"]
                                    funct["variables"] = {"text": None, "letter": None, "variable": None}
                                    __FUNCTIONS__["split"] = funct
                            else:
                                try:
