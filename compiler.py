...
def __getArgsInsideFunction__(line: str, method: str, __locals: dict) -> str:
    ...
    for i in tempValues:
        if i in list(VAR):
            string = ("".join(tempData)).replace(f"[{i}]", VAR[i])
            tempData[0] = string
        else:
            nameError(f"\"{i}\" is not a declared variable.")
    return ", ".join(tempData)
                            ...
                            if str(LINE).startswith("}"):
                                ...
                                if str(line).startswith("return"):
                                    ...
                                    if varTarget in list(__funcVarTemp__):
                                        __VARIABLES__[varTarget] = __funcVarTemp__[varTarget]
                                    else:
                            ...
                            if _M == function:
                                ...
                                for line in funcCode:
                                    ...
                                    for o in __METHODS__:
                                        if o.funcname == lineM:
                                            value = __getArgsInsideFunction__(line, lineM, __funcVarTemp__)
